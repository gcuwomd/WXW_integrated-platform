...
const handleMicroData = async (router: Router) => {
  ...
  // @ts-ignore
  const data = await window.microApp.getData(); // 返回主应用下发的data数据
  //确保一定有data
  ...
  //刷新页面
  if (window.eventCenterForAppNameVite && !store.token) {
    // 主动获取基座下发的数据
...
const mount = async () => {
  ...
  await handleMicroData(router);

  await app.mount("#vite-app");
};
