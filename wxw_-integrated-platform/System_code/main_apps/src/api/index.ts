...
export const formalURL = "https://nc-wxwjc.gcu.edu.cn/api";
export const alovaInstance = createAlova({
  ...
  // 响应钩子，相当于 axios 的响应拦截器
  // 适配后端网关 GatewayAuthenticationFilter 的 Token 刷新机制：
  // 1. Token 过期但 Redis 未过期 → 返回 {code: 205, msg: "Token已刷新", data: 新Token}
  // 2. Token 过期且 Redis 也过期 → 返回 {code: 401, msg: "Token已失效"}
  // 3. 临时用户登录 → 返回 {code: 401, msg: "登陆成功，生成临时token...", data: token}
  responded: onResponseRefreshToken(async (response: any) => {
    const data = await response.json();

    // 处理 Token 刷新（后端返回 205 表示 Token 已刷新）
