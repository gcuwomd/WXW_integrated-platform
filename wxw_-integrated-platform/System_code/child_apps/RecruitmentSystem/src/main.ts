...
// 不再需要 getchildtoken，新网关架构下直接使用主应用传递的 Token
...
// 不再需要调用子应用登录接口获取独立的 Token
const handleMicroData = async (router: Router) => {
  // eventCenterForAppNameVite 是基座添加到window的数据通信对象
  // @ts-ignore
  const data = window.microApp.getData(); // 返回主应用下发的data数据（同步方法，不需要 await）
  //确保一定有data
  console.log("RecruitmentSystem 收到主应用数据:", data);
  if (!data || !data.token) {
    console.log("未收到主应用数据或 token 为空，等待数据...");
    return;
  }
  let information = data.userInformation;
  //用户信息存储
  const store = useStore();
  // 直接使用主应用传递的 Token
  // 子应用通过此 Token 经网关访问 recruitment 微服务 API
  store.token = data.token;
  store.userid = information.id;
  store.departmentId = information.departmentId;
  store.roleId = information.roleId;
  //刷新页面
  if (window.eventCenterForAppNameVite) {
    // 主动获取基座下发的数据
    console.log("你触发了这个刷新");
...
const mount = async () => {
  ...
  // 先挂载应用，确保页面能渲染
  app.mount("#app");
  console.log("微应用 RecruitNewPeople 渲染了");
