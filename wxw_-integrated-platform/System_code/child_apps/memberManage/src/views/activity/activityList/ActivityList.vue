...
import { department, departmentMember, departmentMemberList } from "../../../type/response-data";
...
const { onSuccess: getPeopleSuccess, send: update } = useSQRequest(() =>
...
// 点击部门按钮：发起网络请求获取该部门成员
const handleDeptClick = async (deptId: number | null, deptName: string) => {
  ...
  if (deptId === null) {
    ...
    // 具体部门：请求部门成员
    const memberRes = await getDepMemberList(deptId);
    if (memberRes.data) {
      allTableData.value = memberRes.data.map((member: departmentMember) => ({
        userId: member.id,
...
const fetchDepartments = async () => {
  ...
  if (depListRes.data) {
    departments.value = depListRes.data;
  }
};
onMounted(() => {
  fetchDepartments();
...
<template>
  <el-card style="width: 100%; height: 100%" shadow="never">
    ...
    <!-- 部门筛选按钮 -->
    <div class="department-filter" style="margin-bottom: 12px">
      <el-button
        :type="activeDeptId === null ? 'primary' : ''"
        size="small"
      ...
      <el-button
        ...
        :key="dept.id"
        :type="activeDeptId === dept.id ? 'primary' : ''"
        size="small"
        @click="handleDeptClick(dept.id, dept.name)"
      >{{ dept.name }}</el-button>
    </div>
    <el-table
