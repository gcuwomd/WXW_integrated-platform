...
import { getAllInformation } from "../api/methods/user/user";
...
//获取角色列表
const { onSuccess: getPerRolesSuccess, send: getPerRolesUpdate } = useSQRequest(
  () => getRoles(),
  {
    immediate: false,
  }
);
getPerRolesSuccess((response: any) => {
      ...
      >
        ...
        <el-table-column fixed="right" label="操作" min-width="180">
          <template #default="scope">
            <el-button
              type="primary"
              @click="openDialog('编辑用户', 'edit', scope.row as InfoUser)"
              >编辑</el-button
            >
            <el-button
              type="danger"
