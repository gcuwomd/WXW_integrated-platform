...
interface SummaryMethodProps<T extends EvaluateInfos = EvaluateInfos> {
...
const { onSuccess: getActEvaluateSuccess } = useSQRequest(() =>
  getActEvalute(store.activityId)
);
getActEvaluateSuccess((response: any) => {
...
const getSummary = (param: SummaryMethodProps) => {
  ...
  columns.forEach((column, index) => {
    ...
    // 循环列表数据得到我们的各个行与列的数据
    const values = data.map((item) => Number(item[column.property as keyof EvaluateInfos]));
    // 通过prop找到你需要计算的列。
          ...
          <el-table-column prop="score" label="评价分数">
            <template #default="scope">
              <el-input-number
                ...
                @input="inputVerification(scope.row as EvaluateInfos)"
              />
            </template>
          </el-table-column>
        </el-table-column>
