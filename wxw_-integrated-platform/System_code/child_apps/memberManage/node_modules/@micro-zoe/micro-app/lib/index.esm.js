...
class CSSParser {
    constructor() {
        ...
        this.optionCssSelectors = []; // use this option to include specific selectors, so that they will be affected by scopecss ,like microApp.options.optionCss
    ...
    formatSelector(skip) {
        ...
        return matchRes.replace(/(^|,[\n\s]*)([^,]+)/g, (_, separator, selector) => {
            ...
            const isIsolationRoot = this.optionCssSelectors.some(includeSelector => {
                ...
                if (includeSelector === 'isolation-root')
                    return true;
                return false;
            });
...
function releaseWorkersByApp(appName) {
    var _a;
    (_a = workerInstanceMap.get(appName)) === null || _a === void 0 ? void 0 : _a.forEach(w => w.terminate());
...
const WorkerProxy = new Proxy(originalWorker, {
    construct(Target, args) {
        ...
        if (url && !isSameOrigin(url)) {
            ...
            instance = new Target(workerPath, options);
        }
        else {
            // 如果 scriptURL 是同源的，直接使用原生的 Worker 构造函数
            instance = new Target(scriptURL, options);
        }
