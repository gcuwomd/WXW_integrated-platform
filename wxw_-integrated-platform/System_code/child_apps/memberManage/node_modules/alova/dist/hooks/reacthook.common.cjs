...
const ObjectCls = Object, undefinedValue = undefined, trueValue = true, objectKeys = (obj) => ObjectCls.keys(obj), forEach = (ary, fn) => ary.forEach(fn), mapItem = (ary, callbackfn) => ary.map(callbackfn), typeOf = (arg) => typeof arg;
...
// React的预定义hooks
var ReactHook = {
    create: (initialValue) => react.useState(initialValue),
    export: stateToData,
    ...
    memorize: (fn) => {
        ...
        const fnRef = react.useRef(noop);
        setRef(fnRef, fn);
        return react.useCallback((...args) => refCurrent(fnRef)(...args), []);
    },
    ref: (initialValue) => {
        const refObj = react.useRef(initialValue);
    ...
    effectRequest({ handler, removeStates, saveStates, immediate, frontStates, watchingStates = [] }) {
        // 当有监听状态时，状态变化再触发
        const oldStates = mapItem(watchingStates, s => react.useRef(s)); // 用于对比新旧值
        react.useEffect(() => {
            // 对比新旧状态，获取变化的状态索引
